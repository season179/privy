...
| Audio format | Opus ~24 kbps mono in **Ogg** (`.ogg`), written by our `OpusIO.OggOpusWriter` against Homebrew libopus/libogg, pages force-flushed every ~1s. **Decided by spike 2026-07-30:** CAF/Opus is eliminated — `AVAudioFile` ignored the 24 kbps setting (emitted 213 kbps) and an unfinalized CAF is unreadable ("missing packet table"), violating req 5 | Measured: 1h → 9.98 MB (~10MB/h confirmed); crash-truncated ogg decodes fully; ffmpeg `-c copy` range extraction from a 1h file works (M2 path); `.ogg` is directly API-compatible |
...
- Opus encoding: system libopus 1.6.1 + libogg 1.3.6 (Homebrew, `pkg-config`-resolved SPM system libraries; note opus.pc puts headers at `include/opus`, so the include is `<opus.h>`). `element-hq/swift-ogg` was considered but not needed.
